...
[Renovate](https://docs.renovatebot.com/) is configured via `renovate.json` (npm dependencies, Dockerfile pins, GitHub Actions pins — grouped by non-major/major, no release-age delay, non-majors automerge as soon as CI passes). It needs the [Renovate GitHub App](https://github.com/apps/renovate) installed on this repo to actually run — a one-time setup step for whoever administers the repo.
